<template>
 <div v-if="dictStore.dict" class="grid grid-cols-2 max-[620px]:grid-cols-1 gap-8">
   <div v-for="interview in interviews" :key="`interview-${interview.id}}`">
     ...
     >
       ...
       <div>
         <p class="text-gray-600 text-xs -mt-1">{{ dictStore.dict.positions[interview.position_id] }}</p>
       </div>
       ...
       <div class="relative pt-1">
         <div
           class="overflow-hidden h-3 mb-2 flex rounded-full bg-slate-200"
         >
           <div
...
const colorBarClass = 'shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center'
...
const allQuestionsPassedInInterview = (interviewId: number) => {
 return metrics.value[interviewId].passedQuestions === metrics.value[interviewId].totalQuestions
}
...
const getInterviewBgColor = (interviewId: number) => {
 ...
 if (interviewId in metricss) {
   ...
   return answeredPct > unclearPct && answeredPct > failedPct ? 'bg-[#00AF9B]/10' :
     unclearPct > answeredPct && unclearPct > failedPct ? 'bg-[#FFB800]/10' :
       failedPct > unclearPct && failedPct > answeredPct ? 'bg-[#EF4743]/10' :
         'white'
 }
}
