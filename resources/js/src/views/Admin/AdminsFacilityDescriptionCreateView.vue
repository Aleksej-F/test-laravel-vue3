  ...
  <div v-if="successfulDownload">
    <div class="project-header">
        <div class="wrap2 projects-title">
            <span  class="info-title">{{projects.projectSelect.titleSlider}}</span>
            <input  
              
              v-model= "projects.projectSelect.titleSlider"
            >
        </div>
    </div>
    <div class="wrap ">
        <div class="project-row">
            <div class="project-col">
                <b>Адрес:</b>
                <textarea rows="1" 
                  v-model= "projects.projectSelect.address"
                  class="textarea_input" 
                ></textarea>
                <b>Период работ:</b>
                <textarea rows="1" 
                  v-model= "projects.projectSelect.workPeriod"
                  class="textarea_input" 
                ></textarea>
               
                <b>Материалы:</b>
                <textarea rows="5" 
                  v-model= "projects.projectSelect.materials"
                  class="textarea_input" 
                ></textarea>
            </div>
            <div class="project-col"
            ...
            >
                ...
                <div class="block-button">
                  ...
                  <Button
                    name="Слайдер"
                    title="Перейти к редактированию слайдера"
                    @click.stop="clickToEditSlider(projects.projectSelect.id)"
                  />
                </div>
            </div>
            <div 
        ...
        <div class="project-row">
          <div class="project-col"></div>
          <div class="project-col">
            <div class="project-video"
                v-if="projects.projectSelect.urlVideo && !imgLoadingStore.visible"
            >
                <video  
                  controls="controls" 
                  :poster="'/storage/img/'+projects.projectSelect.urlImg"
                >
                 
                  <source 
                    :src="'/storage/video/'+projects.projectSelect.urlVideo" 
                    type='video/mp4; '
                  >
            ...
            <div class="block-button">
              <Button
                name="Видео"
                title="Перейти к выбору видео"
                @click.stop="clickToEditVideo()"
              />
              <Button
                v-if="projects.projectSelect.urlVideo"
                name="Удалить"
                title="Удалить видео"
                @click.stop="projects.clickToProjectSelectDeleteVideo()"
              />
            </div>
          </div>
        </div>
        <div class="project-row">
            <div class="project-col">
                ...
                <div class="project-button__cont">
                  <a href="/admins-panel/facilities" class="project-button__link">
                      НАЗАД К ПРОЕКТАМ
                  </a>
                </div>
...
<script setup>
    ...
    async function clickToEditVideo() {
      imgLoadingStore.changeVisibility()
      await imgLoadingStore.getFilesListCatalog(`video`)
    }

    function clickToEditSlider(itemId){
      router.push({
        name: 'adminsFacilitySliderCreate', 
        params: { id: itemId, operation:'edit' }
      })
    }
    ...
    onBeforeMount(
      async() => {
        ...
        if (route.params.operation === "edit"){
          ...
          console.log(itemProject.value)
          if (itemProject.value.sliders.length > 0) {
            activeSlider.value = itemProject.value.sliders[0]
    ...
    onUnmounted(() => {
      ...
      successfulDownload.value = false
    })
